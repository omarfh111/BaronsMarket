...
try:
    ...
    import ml.models.model_8_code.models.fix_imports as fix_imports
...
try:
    from ml.models.model_8_code.models.fph import FPH
...
class DTD(SegmentationModel):
    def __init__(self, encoder_name = "resnet18", decoder_channels = (384, 192, 96, 64), classes = 1, device='cpu'):
        ...
        import os
        import sys
        # Create module aliases for compatibility with older checkpoints/imports.
        # Some runtimes do not register 'models.*' keys in sys.modules.
        cur_mod = sys.modules.get(__name__)
        if cur_mod is not None:
            sys.modules.setdefault('dtd', cur_mod)
            sys.modules.setdefault('models.dtd', cur_mod)
        fph_mod = sys.modules.get('ml.models.model_8_code.models.fph') or sys.modules.get('fph')
        if fph_mod is not None:
            sys.modules.setdefault('fph', fph_mod)
            sys.modules.setdefault('models.fph', fph_mod)
        swins_mod = sys.modules.get('ml.models.model_8_code.models.swins') or sys.modules.get('swins')
        if swins_mod is not None:
            sys.modules.setdefault('swins', swins_mod)
            sys.modules.setdefault('models.swins', swins_mod)
            # Some legacy checkpoints were serialized with classes looked up from
            # '__main__' or plain module paths. Expose common Swin classes there.
            ...
            if main_mod is not None:
                for cls_name in (
                    ...
                    if hasattr(swins_mod, cls_name) and not hasattr(main_mod, cls_name):
                        setattr(main_mod, cls_name, getattr(swins_mod, cls_name))
        # Expose VPH for legacy checkpoints that reference dtd.VPH or __main__.VPH.
        main_mod = sys.modules.get('__main__')
        if cur_mod is not None and main_mod is not None and hasattr(cur_mod, 'VPH') and not hasattr(main_mod, 'VPH'):
            setattr(main_mod, 'VPH', getattr(cur_mod, 'VPH'))
        ...
        if device == 'mps':
            self.vph = torch.load(vph_path, map_location=torch.device('cpu'), weights_only=False)
            self.swin = torch.load(swin_path, map_location=torch.device('cpu'), weights_only=False)
        else:
            self.vph = torch.load(vph_path, map_location=device, weights_only=False)
            self.swin = torch.load(swin_path, map_location=device, weights_only=False)
        self.fph = FPH()
